...
def configure_subprocess(
    ...
    with _xpu_device_context(parallel_config):
...
def _xpu_device_context(
    ...
    * **Multi-device worker (TP=2)**: XCCL/oneCCL sees 4 visible devices but is
    ...
    # Build the list of physical device IDs the worker needs.
    ...
    if len(current_devices) <= needed:
        # Already at or below the required count — nothing to change.
        yield
        return
