...
def _dev_sync() -> None:
    """Synchronize current accelerator (XPU/CUDA) for accurate hang debugging."""
    try:
...
class Worker(WorkerBase):
    ...
    def compile_or_warm_up_model(self) -> CompilationTimes:
        ...
        logger.info_once("start  maybe_remove_all_loras")
        self.model_runner.maybe_remove_all_loras(self.model_runner.lora_config)
        logger.info_once("finish  maybe_remove_all_loras")
        # Warmup and tune the kernels used during model execution before
        # cuda graph capture.
        kernel_warmup(self)

        cuda_graph_memory_bytes = 0
        logger.info_once("start capture_model")
        if not self.model_config.enforce_eager:
            cuda_graph_memory_bytes = self.model_runner.capture_model()

        logger.info_once("finish capture_model")
        ...
        if self.use_v2_model_runner:
            ...
            logger.info_once("start warmup_kernels on xpu")
            warmup_kernels(self.model_runner, self.execute_model, self.sample_tokens)
            logger.info_once("finish capture_model on xpu")
            ...
            # We skip EPLB here since we don't want to record dummy metrics
            logger.info_once("start  _dummy_run on xpu")
            hidden_states, last_hidden_states = self.model_runner._dummy_run(
                num_tokens=max_num_reqs,
                skip_eplb=True,
                cudagraph_runtime_mode=CUDAGraphMode.NONE,
            )
            logger.info_once("finish  _dummy_run on xpu")
            if self.model_runner.is_pooling_model:
                logger.info_once("start  _dummy_pooler_run on xpu")
                self.model_runner._dummy_pooler_run(hidden_states)
                logger.info_once("finish  _dummy_pooler_run on xpu")
            else:
                logger.info_once("start  _dummy_sampler_run on xpu")
                self.model_runner._dummy_sampler_run(hidden_states=last_hidden_states)
                logger.info_once("finish  _dummy_sampler_run on xpu")
        ...
        logger.info_once("start  activate_triton_jit_monitor on xpu")
        activate_triton_jit_monitor(
            verbose=self.observability_config.jit_monitor_verbose
        )
        logger.info_once("finish  activate_triton_jit_monitor on xpu")

        # Freeze the worker heap so the GC won't scan static objects
        # (model weights, KV caches, CUDA graphs) during inference.
        logger.info_once("start  freeze_gc_heap on xpu")
        freeze_gc_heap()
        logger.info_once("finish  freeze_gc_heap on xpu")
        logger.info_once("start  maybe_attach_gc_debug_callback on xpu")
        maybe_attach_gc_debug_callback()
        logger.info_once("finish  maybe_attach_gc_debug_callback on xpu")
    ...
    def sample_tokens(
        ...
        _dev_sync()
        logger.info(
            "[DBG][gpu_worker rank=%d] sample_tokens: enter grammar=%s",
            self.rank,
        ...
        _dev_sync()
        logger.info(
            "[DBG][gpu_worker rank=%d] sample_tokens: return type=%s (synced)",
            self.rank,
            type(out).__name__,
        )
        return out
    ...
    def execute_model(
        ...
        _dev_sync()
        logger.info(
            "[DBG][gpu_worker rank=%d] execute_model: enter "
            "total_num_scheduled_tokens=%d num_reqs=%d",
            self.rank,
        ...
        if self._pp_send_work:
            logger.info(
                "[DBG][gpu_worker rank=%d] execute_model: waiting on %d pp_send_work",
                self.rank,
            ...
            self._pp_send_work = []
            logger.info(
                "[DBG][gpu_worker rank=%d] execute_model: pp_send_work cleared",
                self.rank,
            )
        ...
        if forward_pass and not get_pp_group().is_first_rank:
            logger.info(
                "[DBG][gpu_worker rank=%d] execute_model: PP irecv (not first rank)",
                self.rank,
            )
        ...
        with self.annotate_profile(scheduler_output):
            logger.info(
                "[DBG][gpu_worker rank=%d] execute_model: -> model_runner.execute_model",
                self.rank,
            )
            ...
            _dev_sync()
            logger.info(
                "[DBG][gpu_worker rank=%d] execute_model: "
                "<- model_runner.execute_model type=%s (synced)",
                self.rank,
            ...
            if isinstance(
                ...
                logger.info(
                    "[DBG][gpu_worker rank=%d] execute_model: return early",
                    self.rank,
                )
                return output
        ...
        logger.info(
            "[DBG][gpu_worker rank=%d] execute_model: PP isend (not last rank)",
            self.rank,
        )
        ...
        logger.info(
            "[DBG][gpu_worker rank=%d] execute_model: return None (mid-PP rank)",
            self.rank,
        )
        return None
