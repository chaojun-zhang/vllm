...
class GPUModelRunner(LoRAModelRunnerMixin):
    ...
    def execute_model(
        ...
        try:
            _rank = get_tp_group().rank_in_group
        except Exception:
            _rank = -1
        _dev_sync()
        # logger.info(
        #     "[DBG][gpu/model_runner tp_rank=%d] execute_model: enter "
        #     "num_tokens=%d num_reqs=%d dummy=%s",
        #     _rank,
        ...
        if not dummy_run:
            ...
            _dev_sync()
            # logger.info(
            #     "[DBG][gpu/model_runner tp_rank=%d] execute_model: post req-state "
            #     "update (synced)",
            #     _rank,
            # )
            if scheduler_output.total_num_scheduled_tokens == 0:
                ...
                # logger.info(
                #     "[DBG][gpu/model_runner tp_rank=%d] execute_model: "
                #     "kv_connector.no_forward (zero tokens) -> return",
                #     _rank,
                # )
                return empty_output
        ...
        # logger.info(
        #     "[DBG][gpu/model_runner tp_rank=%d] execute_model: "
        #     "-> dispatch_cg_and_sync_dp num_active_loras=%d",
        #     _rank,
        #     num_active_loras,
        # )
        ...
        # logger.info(
        #     "[DBG][gpu/model_runner tp_rank=%d] execute_model: "
        #     "<- dispatch_cg_and_sync_dp (synced) cg_mode=%s",
        #     _rank,
        #     batch_desc.cg_mode,
        # )
        ...
        if not dummy_run:
            ...
            # logger.info(
            #     "[DBG][gpu/model_runner tp_rank=%d] execute_model: -> prepare_inputs",
            #     _rank,
            # )
            ...
            # logger.info(
            #     "[DBG][gpu/model_runner tp_rank=%d] execute_model: "
            #     "<- prepare_inputs (synced) -> prepare_attn",
            #     _rank,
            # )
        ...
        _dev_sync()
        # logger.info(
        #     "[DBG][gpu/model_runner tp_rank=%d] execute_model: pre-forward "
        #     "cg_mode=%s num_tokens_padded=%d",
        #     _rank,
        #     batch_desc.cg_mode,
        ...
        if batch_desc.cg_mode == CUDAGraphMode.FULL:
            ...
            # logger.info(
            #     "[DBG][gpu/model_runner tp_rank=%d] execute_model: -> run_fullgraph",
            #     _rank,
            # )
        ...
        _dev_sync()
        # logger.info(
        #     "[DBG][gpu/model_runner tp_rank=%d] execute_model: post-forward state "
        #     "cached (synced) is_last_pp=%s",
        #     _rank,
        ...
        if not self.is_last_pp_rank:
            ...
            # logger.info(
            #     "[DBG][gpu/model_runner tp_rank=%d] execute_model: return "
            #     "IntermediateTensors (mid-PP)",
            #     _rank,
            # )
            return output_intermediate_tensors
        # logger.info(
        #     "[DBG][gpu/model_runner tp_rank=%d] execute_model: return None "
        #     "(last-PP, awaiting sample)",
        #     _rank,
        # )
        return None
    ...
    def sample_tokens(
        ...
        _dev_sync()
        # logger.info(
        #     "[DBG][gpu/model_runner tp_rank=%d] sample_tokens: enter "
        #     "has_state=%s grammar=%s",
        #     _rank,
        ...
        if not self.is_last_pp_rank:
            ...
            # logger.info(
            #     "[DBG][gpu/model_runner tp_rank=%d] sample_tokens: -> pp_handler.receive",
            #     _rank,
            # )
            all_decode_next = self.pp_handler.receive(input_batch)
            _dev_sync()
            # logger.info(
            #     "[DBG][gpu/model_runner tp_rank=%d] sample_tokens: "
            #     "<- pp_handler.receive (synced)",
            #     _rank,
            # )
            ...
            _dev_sync()
            logger.info(
                "[DBG][gpu/model_runner tp_rank=%d] sample_tokens: return "
                "(non-last PP, synced)",
                _rank,
            )
        ...
        logger.info(
            "[DBG][gpu/model_runner tp_rank=%d] sample_tokens: -> self.sample",
            _rank,
        )
        ...
        _dev_sync()
        logger.info(
            "[DBG][gpu/model_runner tp_rank=%d] sample_tokens: return AsyncOutput "
            "(synced)",
            _rank,
        )
        return async_output
