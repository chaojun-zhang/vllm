...
class XpuCommunicator(DeviceCommunicatorBase):
    def __init__(
        ...
        if "tp" not in unique_name:
            use_outplace_allreduce = False
        else:
            from vllm.distributed.parallel_state import _ENABLE_CUSTOM_ALL_REDUCE

            use_outplace_allreduce = _ENABLE_CUSTOM_ALL_REDUCE

        self.use_outplace_allreduce = use_outplace_allreduce
    ...
    def all_reduce(self, input_: torch.Tensor) -> torch.Tensor:
        output = input_.clone() if self.use_outplace_allreduce else input_
