...
class TestAllReduceRMSNormXPUMxFP8Model(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        z2 = torch.ops._xpu_C.fp8_gemm(
            fp8, self.w[0], self.dtype, scale, self.ws[0], None
        )
...
def sequence_parallelism_pass_on_xpu_mxfp8_model(
    ...
    with set_current_vllm_config(vllm_config):
        ...
        compiled_model = torch.compile(model, backend=backend)
        # The SP pass emits all_gather(fp8) + all_gather(e8m0fnu_scale) for
        # AsyncTPPass to fuse into fused_all_gather_xpu_fp8_matmul later.
        # Without AsyncTP, XCCL rejects e8m0fnu tensors at runtime.
        # Compilation (which populates backend stats) completes before execution,
        # so catch the expected runtime XCCL dtype error and proceed to assertions.
        try:
            compiled_model(hidden_states)
        except (TypeError, RuntimeError) as exc:
            if "Float8_e8m0fnu" not in str(exc) and "XCCL" not in str(exc):
                raise
